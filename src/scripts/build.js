...
build.photo = function (data) {
  ...
  if (data.cameraDate === '1') html += lychee
    .html`<a><span title='Camera Date'>${ build.iconic('camera-slr') }</span>$${ data.sysdate }</a>`
...
build.imageview = function (data, visibleControls) {
  ...
  if (hasMedium === false) {
    ...
    html += lychee
      .html`<img id='image' class='$${ visibleControls===true ? '' : 'full' }' src='$${ data.url }' srcset='$${ data.medium } 1920w, $${ data.url } $${ data.width }w' draggable='false'>`
